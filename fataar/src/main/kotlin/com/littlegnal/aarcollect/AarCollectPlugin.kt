...
import com.littlegnal.aarcollect.kt.aarLog
...
class AarCollectPlugin : Plugin<Project> {
  ...
  override fun apply(p0: Project?) {
    ...
    project.afterEvaluate {
      ...
      resolveArtifacts()
      // 获取 android 配置
      val appExtension: LibraryExtension =
  ...
  private fun checkAndroidPlugin() {
    if (!project.plugins.hasPlugin("com.android.library")) {
      throw ProjectConfigurationException(
          "fat-aar-plugin must be applied in project that has android library plugin!", null
      )
    }
  }
    ...
    embedConf.resolvedConfiguration.firstLevelModuleDependencies.forEach {
      it.moduleArtifacts.forEach { artifact ->
        ...
        if ("aar" == artifact.type || "jar" == artifact.type) {
          aarLog(
              "resolveArtifacts",
              "fat-aar ---> embed : ${artifact.name} : ${artifact.type} : ${artifact
                  .id}"
          )
        } else {
