...
import com.littlegnal.aarcollect.kt.aarInfoLog
import com.littlegnal.aarcollect.kt.processIntoClasses
import com.littlegnal.aarcollect.kt.processIntoJars
import com.littlegnal.aarcollect.kt.resolveBundleDir
...
class AarProcessor(
  ...
  fun processVariant() {
    ...
    val prepareTask: Task =
      project.tasks.findByPath(taskPath) ?: throw  RuntimeException("Can not find task $taskPath!")
  ...
  private fun processAar() {
    ...
    val preBuildTask = project.tasks.findByPath(prepareDependencies) ?: throw RuntimeException(
        "no find " +
            "prepareDependencies"
    )
  ...
  private fun processManifest() {
    ...
    val manifestOutput = project.file(
        "${project.buildDir.path}/intermediates/fat-aar/${variant
    ...
    val manifestsMergeTask = project.tasks.create(
        "merge${variant.name.capitalize()}Manifest",
  ...
  private fun processClassesAndJars() {
    ...
    if (variant.buildType.isMinifyEnabled) {
      ...
      javacTask.doLast {
        ...
        val dustDir =
          project.file("${project.buildDir.path}/intermediates/classes/${variant.dirName}")
      ...
      val syncLibTask: Task = project.tasks.findByPath(taskJarsFor) ?: throw RuntimeException(
          "Can not find task $taskJarsFor!"
      )
      ...
      aarInfoLog(
          "=========== processClassesAndJars     transformClassesAndResourcesWithSyncLibJars "
      )
  ...
  private fun processAssets() {
    val assetsTask: Task = variant.mergeAssets ?: throw RuntimeException(
        "Can not find task in variant.getMergeAssets()!"
    )
